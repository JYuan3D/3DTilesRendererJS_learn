...
function reinstantiateTiles() {
	const url = "http://202.104.149.202:8099/BSQ/Scene_New/3DTiles.json" || "../data/tileset.json";
	...
	dracoLoader.setDecoderPath(
		"https://unpkg.com/three@0.123.0/examples/js/libs/draco/gltf/"
	);
...
function init() {
	...
	secondCamera = new PerspectiveCamera(
		60,
		window.innerWidth / window.innerHeight,
		1,
		4000
	...
	secondControls = new FlyOrbitControls(
		secondCamera,
	...
	thirdPersonControls = new FlyOrbitControls(
		thirdPersonCamera,
		thirdPersonRenderer.domElement
	);
	...
	const rayMesh = new Mesh(
		new CylinderBufferGeometry(0.25, 0.25, 6),
		rayIntersectMat
	);
	...
	const rayRing = new Mesh(
		new TorusBufferGeometry(1.5, 0.2, 16, 100),
		rayIntersectMat
	);
	...
	secondRenderer.domElement.addEventListener(
		"pointermove",
		onPointerMove,
		false
	);
	...
	if (parseFloat(params.raycast) !== NONE && lastHoveredElement !== null) {
		if (lastHoveredElement === renderer.domElement) {
			raycaster.setFromCamera(
				mouse,
				params.orthographic ? orthoCamera : camera
			);
		} else {
			raycaster.setFromCamera(mouse, secondCamera);
		}
...
function render() {
	...
	thirdPersonRenderer.domElement.style.visibility = params.showThirdPerson
		? "visible"
		: "hidden";
	...
	let str = `Downloading: ${tiles.stats.downloading} Parsing: ${
		tiles.stats.parsing
	} Visible: ${tiles.group.children.length - 2}`;
	...
	if (params.enableCacheDisplay) {
		...
		str += `<br/>Cache: ${(100 * cacheFullness).toFixed(2)}% ~${(
			count /
			1000 /
			1000
		).toFixed(2)}mb`;
